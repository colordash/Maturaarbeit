...
// minimum und maximum von motor range
// dynamisches wechseln von idealem speed und VOllt
// reset funktion für PID
...
// Nur während "flip-over" --> aktiviert post-crash und bringt den Motoren Power so , dass die Drohne wieder normal fliegt 
// +interpretation der steuerung / nutzt exponenzielle Reaktion der Steuereingaben
...
// begrenzt die Drehzahl der Motoren und impelementiert den PID-Regelmechanismus
...
// endgültige Motorleistung durch Skalieren bregenzt motorleistung und schaut ob failsafe oder scharfgeestellt
static void applyMixToMotors(float motorMix[MAX_SUPPORTED_MOTORS], motorMixer_t *activeMixer)
{
    // Now add in the desired throttle, but keep in a range that doesn't clip adjusted
    // roll/pitch/yaw. This could move throttle down, but also up for those low throttle flips.
    for (int i = 0; i < mixerRuntime.motorCount; i++) {
        // Throttle wäre der schub den es zu geben gibt
...
#ifdef USE_DYN_LPF
//Filter rechnet nur wenn es grosse änderungen gibt / Filtern von Rauschen
...
#endif
//lineare veränderung der Motorleistung / wenn "airmode" aktiv ist wird mit 50 % Motorleistung gestellt, um eine gleichmässigere Steuerung zu bekommen
// begrenzungen um überschneidungen zu vermeiden
...
//berechnet geschw und grenzwerte für den Gashebel bei der landung
//für sanftere landung und vermeidung von grossen schüben unten am boden 
//prüft ob dies angewandt ewerden soll mit vergleich zu (ezLandingThreshold, ezLandingSpeed)
...
//Hauptmischfunktion, PID, Gaswert --> Leistung von Motoren
...
//winkelbasierter korekturwert für den Gashebel
void mixerSetThrottleAngleCorrection(int correctionValue)
{
    throttleAngleCorrection = correctionValue;
}
//internes Gas für Telemetrie und Feedback
